...
int HongMo_SetFrameRate(int fps)
{
	...
	if(xioctl(fd,VIDIOC_G_PARM, &v_param) < 0)
	{
		//error("set param:");
		ALOGE("failed to set framerate .");
		return -1;
	}

	ALOGE("SetFrameRate bef framerate: %d / %d",v_param.parm.output.timeperframe.numerator,v_param.parm.output.timeperframe.denominator);

	 
	// parm->parm.capture.capturemode = V4L2_MODE_HIGHQUALITY;    
	v_param.parm.capture.timeperframe.denominator = fps;//那㊣??????﹞???
	//v_param.parm.capture.timeperframe.numerator = 1;//﹞?℅車//del by wzb
	 
	if(xioctl(fd,VIDIOC_S_PARM, &v_param) < 0)
	{
		//error("set param:");
		ALOGE("SetFrameRate failed to set framerate .");
		return -1;
	}
	...
	// ?足2谷???那那?﹞?谷豕??3谷1|
	ALOGE("SetFrameRate aft framerate: %d / %d",v_param.parm.output.timeperframe.numerator,v_param.parm.output.timeperframe.denominator);
	return 0;
}
  
    
int HongMo_EnumFormats(void)
{
